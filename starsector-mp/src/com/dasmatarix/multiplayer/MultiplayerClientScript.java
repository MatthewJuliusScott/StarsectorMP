...
public class MultiplayerClientScript implements EveryFrameScript {
	...
	public static void main(String[] args) {
		MultiplayerClientScript client;
		try {
			client = new MultiplayerClientScript(InetAddress.getLocalHost(),
			        7777);
			while (true) {
				client.advance(1);
			}
	...
	// how long since the last read/write, we don't wont to send packets every
	// frame
	private long		lastRun			= System.currentTimeMillis();

	// how many milliseconds to wait since lastRun to send/receive again
	private long		runCyle			= 1000L;
	...
	private Map			responseHandlers		= Collections
	        .synchronizedMap(new HashMap());

	private ResponseHandler handler = new ResponseHandler();

	public MultiplayerClientScript(InetAddress hostAddress, int port)
	        throws IOException {
	...
	public void advance(float amount) {
		try {
			if (System.currentTimeMillis() - lastRun > runCyle) {
